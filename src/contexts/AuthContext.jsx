...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(localStorage.getItem('moodboard_token'));

  const logout = useCallback(() => {
    localStorage.removeItem('moodboard_token');
    setToken(null);
    setUser(null);
  ...
  const localLogin = useCallback(async () => {
    try {
      ...
      if (jwtToken) {
        localStorage.setItem('moodboard_token', jwtToken);
        setToken(jwtToken);
        setUser(userData);
