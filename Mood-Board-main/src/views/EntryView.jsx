...
const EntryView = ({ 
  ...
  const [currentMood, setCurrentMood] = useState(
    editingEntry?.mood ?? selectedMood ?? DEFAULT_MOOD
  );
  ...
  useEffect(() => {
    setCurrentMood(editingEntry?.mood ?? selectedMood ?? DEFAULT_MOOD);
  }, [editingEntry, selectedMood]);
  ...
  const handleGenerateInsights = (tabName) => {
    ...
    const result = getAIInsights(currentMood, content);
    setInsights(result);
    ...
    <div className="entry-container" style={{ marginTop: '1rem', position: 'relative' }}>
      ...
      <div className="entry-grid">
        <div className="entry-left">
          ...
          <div style={{ marginBottom: '1rem' }}>
            <MoodDisplay moodValue={currentMood} />
            {isEditing && (
