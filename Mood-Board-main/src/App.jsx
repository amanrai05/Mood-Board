...
const AppContent = () => {
  ...
  const handleMoodSelect = (moodData) => {
    if (typeof moodData === 'object' && moodData.isAutoDetected) {
      navigate('/dashboard/entry', { state: { mood: moodData.mood, aiResult: moodData.aiResult } });
    } else {
      navigate('/dashboard/entry', { state: { mood: moodData } });
    }
  };
  ...
  const { mood: selectedMood, entry: editingEntry, aiResult } = locationState;
  ...
  useEffect(() => {
    ...
    window.addEventListener('moodboard:new-entry', handler);
    return () => window.removeEventListener('moodboard:new-entry', handler);
  }, [navigate]);
