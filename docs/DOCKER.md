# 🐳 Docker Quick Start Guide for MoodBoard

This guide will help you get MoodBoard running with Docker in just a f### Changing Ports
...
```bash
# Backup your data
docker run --rm -v moodboard_moodboard_data:/data -v $(pwd):/backup alpine tar czf /backup/moodboard-backup.tar.gz -C /data .

# Restore data
docker run --rm -v moodboard_moodboard_data:/data -v $(pwd):/backup alpine tar xzf /backup/moodboard-backup.tar.gz -C /data

# View data volume
docker volume inspect moodboard_moodboard_data
```
...
services:
  api:
    volumes:
      - ./path/to/your/moodboard.db:/app/data/moodboard.db
```
...
docker-compose down
docker volume rm moodboard_moodboard_data
docker-compose up -d
```
