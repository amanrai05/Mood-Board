...
const Sidebar = ({ onLoadStatistics }) => {
  ...
  useEffect(() => {
    if (location.pathname.includes('stats')) {
      if (!loadedRef.current && typeof onLoadStatistics === 'function') {
        loadedRef.current = true;
        onLoadStatistics();
      }
    } else {
      // Reset so stats reload if user navigates away and back
      ...
      <div className="sidebar__inner">
        <div className="sidebar__brand" style={{ alignItems: 'flex-start', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <strong style={{ color: 'var(--text)', letterSpacing: '-0.01em', fontSize: '1.5rem', fontWeight: '700' }}>MoodBoard</strong>
          </div>
