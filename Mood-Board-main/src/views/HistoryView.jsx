...
const HistoryView = ({ pastEntries, loading, error, onMoodSelect, onDelete, onEdit, onAddEntry, onRefreshHistory, renderOnlyHeader = false }) => {
  ...
  const handleSaveVoiceEntry = async () => {
    ...
    try {
      // Ensure we have a local self-host token (race-condition: auto-login may not have completed)
      if (!apiService.token) {
        try {
          const authResp = await apiService.localLogin();
          if (authResp && authResp.token) {
            apiService.setAuthToken(authResp.token);
          }
        } catch (authErr) {
          // Non-fatal; proceed and let createMoodEntry return a clear error
          console.warn('Auto local-login failed before saving voice entry', authErr);
        }
      }
      const now = new Date();
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              <button
                ...
                style={{
                  ...
                  background: 'transparent', color: '#ff5a5f', border: '1px solid #ff5a5f',
              ...
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1rem',
                  background: 'transparent', color: '#ff9f43', border: '1px solid #ff9f43',
                  borderRadius: '999px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600', transition: 'all 0.2s',
                }}
              >
              ...
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1rem',
                  background: 'transparent', color: '#f6e96b', border: '1px solid #f6e96b',
                  borderRadius: '999px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600', transition: 'all 0.2s',
                }}
              >
              ...
              {speechSupported && (
                <button
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1rem',
                    background: isRecording ? '#22c55e' : 'transparent', 
                    color: isRecording ? 'white' : '#4ade80', 
                    border: isRecording ? 'none' : '1px solid #22c55e',
                    borderRadius: '999px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600', transition: 'all 0.2s',
