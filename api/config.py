...
class Config:
    ...
    DATABASE_PATH = os.environ.get("DATABASE_PATH") or os.path.join(
        Path(__file__).parent.parent, "data", "moodboard.db"
    )

    # CORS configuration
    CORS_ORIGINS = os.environ.get(
        "CORS_ORIGINS", "http://localhost:5173,http://localhost:5174,https://moodboard.vercel.app"
    ).split(",")
...
class ProductionConfig(Config):
    ...
    # Use the system's temp directory (works on Windows, Linux, macOS)
    DATABASE_PATH = os.environ.get("DATABASE_PATH") or os.path.join(
        tempfile.gettempdir(), "moodboard.db"
    )
...
class TestingConfig(Config):
    ...
    DEBUG = True
    TESTING = True
    # Use a file-backed SQLite DB in the system's temp directory so multiple connections see the same data
    DATABASE_PATH = os.path.join(tempfile.gettempdir(), "moodboard_test.db")
