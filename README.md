...
<img src="https://raw.githubusercontent.com/shirsakm/moodboard/refs/heads/dev/public/logo.png" height="60px" />
<h1>MoodBoard</h1>

[![GitHub license](https://img.shields.io/github/license/shirsakm/moodboard?style=flat-square)](https://github.com/shirsakm/moodboard/blob/main/LICENSE)
[![GitHub stars](https://img.shields.io/github/stars/shirsakm/moodboard?style=flat-square)](https://github.com/shirsakm/moodboard/stargazers)
[![GitHub Actions Workflow Status](https://img.shields.io/github/actions/workflow/status/shirsakm/moodboard/publish.yml?branch=master&style=flat-square)](https://github.com/shirsakm/moodboard/actions)
![GitHub last commit](https://img.shields.io/github/last-commit/shirsakm/moodboard)
![GitHub Tag](https://img.shields.io/github/v/tag/shirsakm/moodboard)
...
MoodBoard was inspired by awesome mood-tracking apps like Daylio, but born out of frustration with aggressive subscription models, paywalls, and a lack of cross-platform access. I wanted a beautiful, effective tool to log my mood and journal my thoughts without compromising on privacy or being locked into a single device.

MoodBoard is the result: a feature-complete, open-source alternative that you can run anywhere. It's fully web-based and responsive for use on both desktop and mobile. No ads, no subscriptions, and absolutely no data mining. Just you and your data.
...
> [!NOTE]
> By default, MoodBoard runs in **single-user mode**. Enable Google OAuth for multi-user support.

Get your own MoodBoard instance running in under 5 minutes.

```bash
# 1. Clone the repository
git clone https://github.com/shirsakm/moodboard.git
cd moodboard
...
```bash
docker network create moodboard-test || true
docker run -d --name moodboard-api \
    --network moodboard-test --network-alias api \
    -e SECRET_KEY=$(openssl rand -hex 32) \
    -e JWT_SECRET=$(openssl rand -hex 32) \
    -e CORS_ORIGINS=http://localhost:5173 \
    -e ENABLE_GOOGLE_OAUTH=0 \
    -e DEFAULT_SELF_HOST_ID=selfhost_default_user \
    -e DATABASE_PATH=/app/data/moodboard.db \
    -e PORT=5000 \
    -v moodboard_data:/app/data \
    ghcr.io/shirsakm/moodboard-api:latest

docker run -d --name moodboard-frontend \
    --network moodboard-test \
    -p 5173:80 \
    ghcr.io/shirsakm/moodboard-frontend:latest
```
...
```bash
export API_IMAGE=ghcr.io/shirsakm/moodboard-api:latest
export WEB_IMAGE=ghcr.io/shirsakm/moodboard-frontend:latest
mkdir -p data
...
services:
  api:
    image: ghcr.io/shirsakm/moodboard-api:latest
    ...
    environment:
      ...
      - DATABASE_PATH=/app/data/moodboard.db
      - PORT=5000
    volumes:
      - ./data:/app/data
    expose:
      - "5000"
    networks: { moodboard: { aliases: [api] } }
  web:
    image: ghcr.io/shirsakm/moodboard-frontend:latest
    restart: unless-stopped
    depends_on: [api]
      ports:
...
> [!NOTE]
> 1. Persistent data lives in `./data/moodboard.db` — include it in backups.
...
You can customize your MoodBoard instance using environment variables in the `.env` file.
...
MoodBoard is actively developed. Here are some of the features planned for the future:
